...
app.get("/forecast/:location/:numberOfDays", async (c) => {
    const {weatherApiKey} = c.get('config')
    var location = c.req.param("location")
    var numberOfDays = c.req.param("numberOfDays")
    var result = await axios.get(`https://api.weatherapi.com/v1/forecast.json?key=${weatherApiKey}&q=${location}&days=${numberOfDays}&aqi=yes&alerts=no`)
...
app.get("/forecast/:latitude/:longitude", async (c) => {
    ...
    var result = await axios.get(`http://api.weatherapi.com/v1/forecast.json?key=${weatherApiKey}&q=${latitude},${longitude}&aqi=yes`)
    var forecast = result.data
    return c.json(forecast)
})
...
app.get("/search/:location", async (c) => {
    const {weatherApiKey} = c.get('config')
    var location = c.req.param("location")
    var result = await axios.get(`https://api.weatherapi.com/v1/search.json?key=${weatherApiKey}&q=${location}`)
