...
app.get("/archive/:year/:month", async (c) => {
    console.log("Archive");
    const {NYTIMESAPIKey} = c.get('config')
    const year: number = parseInt(c.req.param("year"));
    const month: number = parseInt(c.req.param("month"));
    
    const result = await axios.get(`https://api.nytimes.com/svc/archive/v1/${year}/${month}.json?api-key=${NYTIMESAPIKey}`);
...
app.get(`/articles/search/:q`, async (c) => {
    ...
    try {
        ...
        let url = `https://api.nytimes.com/svc/search/v2/articlesearch.json?q=${q}&api-key=${NYTIMESAPIKey}`;
        if (fq) url += `&fq=${fq}`;
...
app.get("bestsellers/books/:list-name", async (c) => {
    ...
    let url : string = `https://api.nytimes.com/svc/books/v3/lists/${list_name}.json?api-key=${NYTIMESAPIKey}`;
...
app.get("bestsellers/books/:bestseller-date/:list-name", async (c) => {
    ...
    let url : string = `https://api.nytimes.com/svc/books/v3/lists/${bestsellers_date}/${list_name}.json?api-key=${NYTIMESAPIKey}`;
    if (offset) url += `&offset=${offset}`;
...
app.get("bestsellers/books/names", async (c)  => {
    const {NYTIMESAPIKey} = c.get('config')
    const result = await axios.get(`https://api.nytimes.com/svc/books/v3/lists/names.json?api-key=${NYTIMESAPIKey}`);
    return c.json(result.data);
});
...
app.get("bestsellers/books/overview", async (c) => {
    const {NYTIMESAPIKey} = c.get('config')
    const published_date: string = c.req.query("published-date") || "";
    let url = `https://api.nytimes.com/svc/books/v3/lists/full-overview.json?api-key=${NYTIMESAPIKey}`;
...
app.get("bestsellers/books/overview/top5", async (c) => {
    const {NYTIMESAPIKey} = c.get('config')
    const published_date: string = c.req.query("published-date") || "";
    let url = `https://api.nytimes.com/svc/books/v3/lists/best-sellers/overview.json?api-key=${NYTIMESAPIKey}`;
    if (published_date) url += `&published_date=${published_date}`;
    const result = await axios.get(url);
    return c.json(result.data);
})
...
app.get("popular/viewed/:period", async (c) => {
    ...
    const result = await axios.get(`https://api.nytimes.com/svc/mostpopular/v2/viewed/${period}.json?api-key=${NYTIMESAPIKey}`);
...
app.get("popular/shared/:period", async (c) => {
    const {NYTIMESAPIKey} = c.get('config')
    const period: string = c.req.param("period");
    const result = await axios.get(`https://api.nytimes.com/svc/mostpopular/v2/shared/${period}.json?api-key=${NYTIMESAPIKey}`);
...
app.get("popular/shared/:period/:source", async (c) => {
    ...
    const result = await axios.get(`https://api.nytimes.com/svc/mostpopular/v2/shared/${period}/${source}.json?api-key=${NYTIMESAPIKey}`);
...
app.get("topstories/:section", async (c) => {
    ...
    const result = await axios.get(`https://api.nytimes.com/svc/topstories/v2/${section}.json?api-key=${NYTIMESAPIKey}`);
