...
app.get("/search/:videotitle", async (c) => {
    const {youtubeApiKey} = c.get('config')
    var video = c.req.param("videotitle")
    var result = await axios.get(`https://www.googleapis.com/youtube/v3/search?key=${youtubeApiKey}&type=video&part=snippet&q=${video}`)
