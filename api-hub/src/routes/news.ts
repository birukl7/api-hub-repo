...
app.get("/search/:term", async (c) => {
    const {NewsAPIKey} = c.get('config')
    var term = c.req.param("term")
    var today = moment()
    var sevenDaysAgo = moment(today).subtract(7, 'days');
    var sort = "popularity";
    var fromDate = `${sevenDaysAgo.year()}-${(sevenDaysAgo.month() + 1).toString().padStart(2,"0")}-${sevenDaysAgo.date().toString().padStart(2,"0")}`;
    var result = await axios.get(`https://newsapi.org/v2/everything?q=${term}&from=${fromDate}&sortBy=${sort}&apiKey=${NewsAPIKey}`)
